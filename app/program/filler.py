...
Both are filtered by the no-repeat window. If *everything* played recently (small local
library), a random candidate is played anyway: repeating a song beats going silent.
"""
...
class FillerSource:
    ...
    def next_segment(self, config: dict) -> tuple[Segment, str] | None:
        ...
        for entry in load_reserve(self.reserve_path)["tracks"]:
        ...
        # Everything played recently: a repeat beats silence.
        chosen = self.rng.choice(fresh or candidates)
